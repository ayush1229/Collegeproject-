...
export default function ReturnPage() {
  ...
  function handleReturn(id) {
    setData(prev =>
      prev.map(s =>
        s.id === id
          ? { ...s, is_returned: true, datetime: new Date().toISOString() }
          : s
      )
    );
  }
  ...
  const filtered = useMemo(() => {
    ...
    // ⬆️ NOT RETURNED FIRST
    list.sort((a, b) => Number(a.is_returned) - Number(b.is_returned));
        ...
        >
          <div>
            ...
            <span className={`inline-block mt-1 px-2 py-1 text-xs rounded-full font-semibold ${
              ...
              {o.is_returned ? "RETURNED" : "OUTSIDE CAMPUS"}
            </span>
          </div>
          ...
          {!o.is_returned ? (
            <button
              ...
              className="bg-green-600 text-white px-4 py-2 text-xs rounded-lg"
            >
              Mark Return
            </button>
          ) : (
